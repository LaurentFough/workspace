...
" "for"
syn region   ocamlNone matchgroup=mineocamlRepeat start="\<for\>" matchgroup=ocamlKeyword end="\<\(to\|downto\)\>" contains=ALLBUT,@ocamlContained,ocamlCountErr
...
" "if"
syn region   ocamlNone matchgroup=mineocamlConditional start="\<if\>" matchgroup=mineocamlConditional end="\<then\>" contains=ALLBUT,@ocamlContained,ocamlThenErr
...
syn keyword mineocamlConditional match with else
...
if version >= 508 || !exists("did_ocaml_syntax_inits")
  ...
  HiLink mineocamlConditional Conditional
  HiLink mineocamlRepeat Repeat
